...
/** 
 * The Hungarian Algorithm tries to minimize the cost of an assignment.
 * Note that, to minimize the cost of a similarity assignment, similarities
 * must be near 1, not near 0 as in the typical worker-task example.
 * This class utilizes the {@link HungarianAlg} written by Konstantinos A. Nedas
 * @author stefan.schaefer
 *
 */
public class HungarianAlgorithm extends AbstractAssignment {
	/**
	 ...
	 * 
	 */
	private static class HungarianAlg {
		//*******************************************//
	...
	public ISimilarityMatrix<IEntity> assignWithEquallySizedSets(ISimilarityMatrix<IEntity> similarities) {
		...
		IEntity[] firstSet = new IEntity[similarities.getFirstSet().size()];
		IEntity[] secondSet = new IEntity[similarities.getSecondSet().size()];
		similarities.getFirstSet().toArray(firstSet);
		similarities.getSecondSet().toArray(secondSet);
		
		double[][] similarityArray = new double[firstSet.length][secondSet.length];
		for (int i = 0; i < firstSet.length; i++) {
			for (int j = 0; j < secondSet.length; j++) {
				similarityArray[i][j] = similarities.getSimilarities(firstSet[i], secondSet[j])
														.iterator().next().similarity;
			}
		}
		int[][] assignments = HungarianAlg.hgAlgorithm(similarityArray, "max");
		final int first = 0;
		final int second = 1;
