...
import de.uni_potsdam.hpi.bpt.promnicat.utilityUnits.processMetrics.ModelStructuringUnit;
...
public class UnitChainBuilder implements IUnitChainBuilder {
	 ...
	 */
	public UnitChainBuilder(String pathToConfig, Class<?> unitDataType) throws IOException {
		this(new ConfigurationParser(pathToConfig), unitDataType);
	}
	
	/**
	 * Only a delegating constructor.
	 */
	private UnitChainBuilder(ConfigurationParser configParser, Class<?> unitDataType){
		this(configParser.getDbInstance(), configParser.getThreadCount(), unitDataType);
	}
	...
	@Override
	public void addUnit(Class<? extends IUnit<IUnitData<Object>, IUnitData<Object>>> clazz, Class<?>[] parameterTypes, List<?> parameters) {
		if(parameterTypes.length != parameters.size()) {
			throw new IllegalArgumentException("Size of paramter type list must equal those of the parameters list!");
		}
		try {
			IUnit<IUnitData<Object>, IUnitData<Object>> unit = clazz.getConstructor(parameterTypes).newInstance(parameters.toArray());
			checkForCompatibility(unit);
		} catch (Exception e) {
			// TODO Auto-generated catch block, inform user about failing
			// instantiation or throw error up
			e.printStackTrace();
		}
	}	
	...
	@Override
	public void createProcessModelMetricsCalulatorUnit(Collection<ProcessMetricConstants.METRICS> metricsToCalculate, boolean handleSubProcesses) throws IllegalTypeException {		ProcessModelMetricsCalculatorUnit metricsUnit = new ProcessModelMetricsCalculatorUnit(metricsToCalculate, handleSubProcesses);
		checkForCompatibility(metricsUnit);	
	}
